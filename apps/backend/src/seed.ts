...
async function main() {
  try {
    const dbUrl = (drizzleConfig as any)?.dbCredentials?.url || './.wrangler/state/v3/d1/database.sqlite';
    ...
    if (better) {
      ...
      const getRole = db.prepare('SELECT id FROM user_role WHERE user_id = ? AND role = ?');
      ...
      console.log('Seeding finished: admin and user created (or already existed) with roles.');
      db.close();
      return;
    }
    ...
    if (sqlite3mod) {
      ...
      const adminRow = await getAsync('SELECT id FROM user WHERE email = ?', ['admin@gmail.com']);
      if (adminRow) {
        const roleRow = await getAsync('SELECT id FROM user_role WHERE user_id = ? AND role = ?', [adminRow.id, 'admin']);
        if (!roleRow) {
          await runAsync('INSERT INTO user_role (user_id, role) VALUES (?, ?)', [adminRow.id, 'admin']);
        }
      }
      ...
      const userRow = await getAsync('SELECT id FROM user WHERE email = ?', ['user@gmail.com']);
      if (userRow) {
        const roleRow = await getAsync('SELECT id FROM user_role WHERE user_id = ? AND role = ?', [userRow.id, 'user']);
        if (!roleRow) {
          await runAsync('INSERT INTO user_role (user_id, role) VALUES (?, ?)', [userRow.id, 'user']);
        }
      }
    ...
    throw new Error('No supported sqlite client found. Install better-sqlite3 or sqlite3.');
  } catch (err) {

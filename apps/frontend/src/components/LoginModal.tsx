...
import { X, KeyRound, Mail, LogIn, Loader2 } from 'lucide-react';
...
export const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleLocalSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Just close modal on success, Navbar will display Dashboard button dynamically.
      onClose();
    } catch (err: any) {
          ...
          <form onSubmit={handleLocalSubmit} className="space-y-5">
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-4 bg-white dark:bg-background border border-slate-400 dark:border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-lg md:text-xl font-bold text-black dark:text-foreground disabled:opacity-60 transition-all placeholder:text-slate-500"
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <KeyRound className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-500 dark:text-slate-400" />
                  <input
                    type="password"
                    placeholder="••••••••"
                    required
                    ...
                    className="w-full pl-11 pr-4 py-4 bg-white dark:bg-background border border-slate-400 dark:border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-lg md:text-xl font-bold tracking-widest text-black dark:text-foreground disabled:opacity-60 transition-all placeholder:text-slate-500"
                  />
                </div>
              </div>
            </div>

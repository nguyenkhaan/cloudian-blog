  ...
  return (
    <aside className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-150 p-5 shadow-sm space-y-3">
        <h4 className="font-bold text-slate-800 text-sm">Permanent Link (Slug)</h4>
        <input
          type="text"
          placeholder="abbreviated-title"
          value={slug}
          onChange={(e) => onSlugChange(e.target.value)}
          className="w-full px-3 py-2 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 text-xs text-slate-700 bg-white"
        />
        <p className="text-[10px] text-slate-400">If left blank, slug will be generated automatically from the title.</p>
      </div>
    </aside>
  );
};
